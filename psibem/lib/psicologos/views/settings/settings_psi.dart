...
class SettingsPsicologo extends StatefulWidget {
  const SettingsPsicologo({super.key});

  @override
  _SettingsPsicologoState createState() => _SettingsPsicologoState();
}

class _SettingsPsicologoState extends State<SettingsPsicologo> {
  String _uid = '';
  String _email = '';
  String _apelido = '';
  String _crp = '';
  ...
  final Map<String, bool> _especialidades = {
    "Psicanálise": false,
    "Terapia Cognitivo-Comportamental": false,
    "Autismo": false,
    "Depressão": false,
    "Ansiedade": false,
  };
        ...
        setState(() {
          ...
          if (data['especialidades'] != null) {
            final especialidades =
                data['especialidades'] as Map<String, dynamic>;
            for (var key in _especialidades.keys) {
              if (especialidades.containsKey(key)) {
                _especialidades[key] = especialidades[key] ?? false;
              }
            }
          }
        });
  ...
  Future<void> _syncUserData() async {
    try {
      ...
      await _firestore.collection('usuarios').doc(user.uid).set({
        ...
        'crp': _crp.isNotEmpty ? _crp : 'Crp em análise',
  ...
  // Função para exibir o AlertDialog de seleção de especialidades
  Future<void> _mostrarSelecaoEspecialidades() async {
    // Cópia temporária das especialidades para edição no AlertDialog
    Map<String, bool> especialidadesTemp = Map.from(_especialidades);
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setStateDialog) {
            return AlertDialog(
              title: const Text('Selecione as especialidades'),
              content: SingleChildScrollView(
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: especialidadesTemp.keys.map((String especialidade) {
                    return CheckboxListTile(
                      title: Text(especialidade),
                      value: especialidadesTemp[especialidade],
                      onChanged: (bool? value) {
                        setStateDialog(() {
                          especialidadesTemp[especialidade] = value!;
                        });
                      },
                    );
                  }).toList(),
    ...
    if (result == true) {
      setState(() {
        ...
        _especialidades.addAll(especialidadesTemp);
      });
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Especialidades atualizadas com sucesso!')),
          );
        }
      } catch (e) {
        if (mounted) {
          _showErrorSnackbar('Erro ao salvar especialidades: $e');
        }
      }
    }
  }
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  // Adiciona a seção de especialidades
                  ListTile(
                    leading: const Icon(Icons.medical_services),
                    title: const Text('Especialidades'),
                    subtitle: Text(
                      _especialidades.entries
                          .where((e) => e.value)
                          .map((e) => e.key)
                          .join(', '),
                    ),
                    onTap: _mostrarSelecaoEspecialidades,
                  ),
                  const Divider(),
                                    ...
                                    child: SizedBox(
                                      height:
                                          MediaQuery.of(context).size.height *
                                              0.9,
                                      child:
                                          TermsOfUseContent(), // Extrai o conteúdo para um widget separado
                                    ),
                                  ),
                                );
                              },
